...
def get_returnPeriod_data(comid):
    request_params = dict(comid=comid, cty='bangladesh')
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/hiwatAPI/getreturnPeriodHIWAT', params=request_params, headers=request_headers)
...
def get_historic_data(comid):
    ...
    request_params = dict(comid=comid, cty='bangladesh')
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/hiwatAPI/getHistoricHIWAT', params=request_params,headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'hdates':
            hdates = value
...
def get_flow_duration_curve(comid):
    ...
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/api/getFlowDurationCurve', params=request_params,headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'prob':
            prob = value
...
def getForecastData(comid):
    request_params = dict(comid=comid, cty='bangladesh')
    request_headers = dict(Authorization=AuthorizationToken)
    res = requests.get(APIHost+'/apps/apicenter/hiwatAPI/getForecastHIWAT', params=request_params, headers=request_headers)
    data = json.loads(res.text)
    for key, value in data.items():
        if key == 'dates':
            dates = value
